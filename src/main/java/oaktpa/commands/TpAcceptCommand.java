...
public class TpAcceptCommand implements CommandExecutor {
    ...
    public TpAcceptCommand(OakTpa plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (requester == null || !requester.isOnline()) {
            ...
            player.sendMessage(plugin.getConfigManager().getPrefix() + 
                    plugin.getConfigManager().getMessage("player-not-found", "player", "requester"));
            return true;
        }
        
        player.sendMessage(plugin.getConfigManager().getPrefix() + 
                plugin.getConfigManager().getMessage("request-accepted-receiver", "player", requester.getName()));
        requester.sendMessage(plugin.getConfigManager().getPrefix() + 
                plugin.getConfigManager().getMessage("request-accepted-sender", "player", player.getName()));
        ...
        Player teleportingPlayer = request.isTpaHere() ? player : requester;
        Player otherPlayer = request.isTpaHere() ? requester : player;
        
        plugin.getTeleportManager().scheduleTeleport(teleportingPlayer, destination, otherPlayer);
